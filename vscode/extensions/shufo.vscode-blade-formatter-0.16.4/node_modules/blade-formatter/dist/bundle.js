...
`)}`:/(@button|@class|@include|@disabled|@checked)/gi.test(s)?i.default.replace(s,/(?<=@(button|class|include|disabled|checked).*?\()(.*)(?=\))/gis,(p,d,y)=>{let x=this.wrapLineLength;["button","class"].includes(d)&&(x=80),d==="include"&&(x=this.wrapLineLength-"func".length-r.length-c.amount);let f=I(`func(${y})`,x,!0).replace(/([\n\s]*)->([\n\s]*)/gs,"->").replace(/,(\s*?\))/gis,(S,L)=>L).trim();return this.isInline(f)?`${this.indentRawPhpBlock(c,`${f}`).replace(/func\((.*)\)/gis,(S,L)=>L).trim()}`:this.indentRawPhpBlock(c,`${f}`).replace(/func\((.*)\)/gis,(S,L)=>L).trim()}):`${I(s).trimEnd()}`}))}restoreRawPhpTags(e){return new Promise(t=>t(e)).then(t=>i.default.replace(t,new RegExp(`${this.getRawPhpTagPlaceholder("(\\d+)")}`,"gms"),(n,r)=>{var s;try{let a=this.rawPhpTags[r],c=/(?<=<\?php\s*?)\/\*.*?\*\/(?=\s*?\?>)/gim.test(a)&&this.isInline(a),h=this.getRawPhpTagPlaceholder(r),u=(s=e.match(new RegExp(`^(.*?)${h}`,"gmi")))!=null?s:[""],p=(0,w.default)(u[0]);if(c)return a;let d=D(this.rawPhpTags[r]).trim().trimRight(`
