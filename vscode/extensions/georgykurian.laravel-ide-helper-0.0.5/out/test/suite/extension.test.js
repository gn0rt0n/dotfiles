...
suite("Extension Test Suite", () => {
    ...
    test("Extension loaded", () => {
        const myExtension = vscode.extensions.getExtension("georgyKurian.laravel-ide-helper");
        assert.notStrictEqual(myExtension, undefined);
        assert.strictEqual(myExtension === null || myExtension === void 0 ? void 0 : myExtension.isActive, true);
    });
    ...
    test("Commands loaded", async () => {
        ...
        extensionCommands.forEach((extensionCommand) => assert.strictEqual(commands.includes(extensionCommand), true, `Couldnt find ${extensionCommand}`));
    });
